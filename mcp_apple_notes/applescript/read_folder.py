...
class ReadFolderOperations(BaseAppleScriptOperations):
    ...
    @staticmethod
    async def read_folder_by_id_and_name(folder_id: str, folder_name: str) -> Dict[str, Any]:
    ...
    @staticmethod
    def _parse_read_by_id_and_name_result(result: str, folder_id: str) -> Dict[str, Any]:
                ...
                if len(parts) >= 6:
                    ...
                    for i in range(child_folder_start, child_folder_end, 2):
                        if i + 1 < len(parts):
                            ...
                            child_folder_primary_key = NoteIDUtils.extract_primary_key(child_folder_full_id)
                    ...
                    for i in range(note_start, len(parts), 2):
                        if i + 1 < len(parts):
                            ...
                            note_primary_key = NoteIDUtils.extract_primary_key(note_full_id)
                            notes.append({
                                'name': note_name,
                                'note_id': note_primary_key
                            })
