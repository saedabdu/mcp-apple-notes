...
class DeleteNoteOperations(BaseAppleScriptOperations):
    ...
    @staticmethod
    async def delete_note_by_id(note_id: str, folder_path: str = "Notes") -> Dict[str, str]:
        ...
        # Perform the deletion using the full note ID
        script = f'''
        tell application "Notes"
            try
                ...
                return "success:" & noteName & ", " & noteId & ", " & creationDate & ", " & modificationDate
            on error errMsg
                return "error:" & errMsg
            end try
        end tell
        ...
        return DeleteNoteOperations._parse_delete_by_id_result(result, folder_path, note_id)
    
    @staticmethod
    def _parse_delete_by_id_result(result: str, folder_path: str, note_id: str) -> Dict[str, str]:
        """Parse the AppleScript result for delete by ID and return deletion information."""
        try:
            # The result format is: success:name, full_id, creation_date, modification_date
