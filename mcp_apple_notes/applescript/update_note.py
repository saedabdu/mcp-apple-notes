...
class UpdateNoteOperations(BaseAppleScriptOperations):
    ...
    @staticmethod
    async def update_note_by_id_and_name(note_id: str, note_name: str, combined_content: str) -> Dict[str, str]:
        ...
        if not combined_content or not combined_content.strip():
            raise ValueError("Combined content cannot be empty or contain only whitespace")
        ...
        return UpdateNoteOperations._parse_update_by_id_and_name_result(result, note_id, note_name)
    
    @staticmethod
    def _parse_update_by_id_and_name_result(result: str, note_id: str, note_name: str) -> Dict[str, str]:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to parse update by ID and name result: {str(e)}")
