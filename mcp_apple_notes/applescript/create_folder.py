...
class CreateFolderOperations(BaseAppleScriptOperations):
    ...
    async def create_folder(folder_name: str, folder_path: str = "") -> Dict[str, Any]:
        ...
        if not folder_path.strip():
            ...
            try:
                ...
                return await CreateFolderOperations._create_nested_folder(folder_name, folder_path)
    ...
    async def _create_root_folder(folder_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.startswith("name:"):
                ...
                return {
                    'name': name,
                    'id': "" # No ID for old format fallback
                }
        except Exception as e:
            raise RuntimeError(f"Failed to parse folder creation result: {str(e)}")
    
    @staticmethod
    async def _create_nested_folder(folder_name: str, folder_path: str) -> Dict[str, Any]:
        """Create folder in nested path."""
        path_components = [comp.strip() for comp in folder_path.split('/') if comp.strip()]
        ...
        for i, component in enumerate(path_components):
            escaped_component = ValidationUtils.create_applescript_quoted_string(component)
            if i == 0:
                path_navigation.append(f"set targetFolder to folder {escaped_component}")
            else:
                path_navigation.append(f"set targetFolder to folder {escaped_component} of targetFolder")
    ...
    async def create_folder_in_existing_path(folder_path: str) -> Dict[str, Any]:
        ...
        if len(path_components) > 1:
            ...
            return await CreateFolderOperations._create_nested_folder(folder_name, parent_path)
        else:
