...
class ListNotesByFolderOperations(BaseAppleScriptOperations):
    ...
    async def list_notes_by_folder(folder_name: str) -> List[Dict[str, str]]:
        """Get all notes from a specific folder (backward compatibility)."""
        script = f'''
    ...
    @staticmethod
    async def list_notes_by_folder_path(folder_path: str) -> List[Dict[str, str]]:
        """Get all notes from a specific folder using path notation."""
        # First resolve the folder path
        folder_info = await FolderPathUtils.resolve_folder_path(folder_path)
        
        # Then list notes in that folder
        script = f'''
        tell application "Notes"
            try
                ...
                repeat with theNote in notes of currentFolder
                    set noteInfo to {{name:(name of theNote), folder:"{folder_path}", creation_date:(creation date of theNote as string), modification_date:(modification date of theNote as string)}}
                    copy noteInfo to end of noteList
                end repeat
                return noteList
            on error errMsg
        ...
        # Check if there was an error
        if result.startswith("error:"):
            raise RuntimeError(f"Failed to list notes by folder path: {result[6:]}")
