            ...
            if line.startswith('Root Folder:'):
                ...
                if '(ID:' in line:
                    ...
                    if '├── Subfolder:' in name_part:
                        ...
                        # Level 2: │   ├──
                        folder_name = name_part.replace('├── Sub-subfolder:', '').strip()
                        formatted_line = f"│   ├── {folder_name} (ID: {primary_key})"
                    elif '├── Sub-sub-subfolder:' in name_part:
                        # Level 3: │   │   ├──
                        folder_name = name_part.replace('├── Sub-sub-subfolder:', '').strip()
                        formatted_line = f"│   │   ├── {folder_name} (ID: {primary_key})"
                    elif '├── Sub-sub-sub-subfolder:' in name_part:
                        # Level 4: │   │   │   ├──
                        folder_name = name_part.replace('├── Sub-sub-sub-subfolder:', '').strip()
                        formatted_line = f"│   │   │   ├── {folder_name} (ID: {primary_key})"
                    else:
